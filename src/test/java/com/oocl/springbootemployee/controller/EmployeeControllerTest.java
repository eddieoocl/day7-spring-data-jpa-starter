...
class EmployeeControllerTest {
    ...
    void setUp() {
        ...
        setUpEmployeeInMemoryRepository();
    }

    private void setUpEmployeeRepository() {
    ...
    void should_create_employee_success() throws Exception {
        ...
        assertThat(employees.get(employees.size() - 1).getName()).isEqualTo(givenName);
        assertThat(employees.get(employees.size() - 1).getAge()).isEqualTo(givenAge);
        assertThat(employees.get(employees.size() - 1).getGender()).isEqualTo(givenGender);
        assertThat(employees.get(employees.size() - 1).getSalary()).isEqualTo(givenSalary);
    }

    @Test
    void should_update_employee_success() throws Exception {
        // Given
        Employee targetEmployee = employeeRepository.findAll().get(0);
        String givenName = "New Employee";
        Integer givenAge = 30;
        Gender givenGender = Gender.FEMALE;
        ...
        // When
        // Then
        client.perform(MockMvcRequestBuilders.put("/employees/" + targetEmployee.getId())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(givenEmployee)
                )
                .andExpect(MockMvcResultMatchers.status().isOk())
        ...
        assertThat(employees.get(0).getId()).isEqualTo(targetEmployee.getId());
